  ...
  return (
    <aside className="w-full md:w-64 lg:w-72 bg-slate-900 text-slate-100 flex flex-col border-r border-slate-800 shrink-0 select-none">
      ...
      <div className="p-4 border-b border-slate-800/80 space-y-3">
        ...
        {/* View Mode Switcher (Single Section vs All Sections) */}
        <div className="pt-1 flex items-center justify-between text-[11px]">
          ...
          <button
            ...
            title="Toggle between single section focus and full continuous view"
          >
            {viewMode === 'single' ? (
              <>
        ...
        {sections.map((sec) => {
          ...
          return (
            <button
              ...
              className={`w-full text-left p-3 rounded-2xl transition-all cursor-pointer border flex flex-col gap-1.5 relative group ${
                isActive
                  ? isEmployment
                    ? 'bg-emerald-950/80 border-emerald-600 text-white shadow-md shadow-emerald-950/40 ring-1 ring-emerald-500/50'
                    : 'bg-blue-950/80 border-blue-600 text-white shadow-md shadow-blue-950/40 ring-1 ring-blue-500/50'
            ...
            >
              ...
              {isActive && (
                <div 
                  className={`absolute left-0 top-2 bottom-2 w-1 rounded-r-full ${
                    isEmployment ? 'bg-emerald-400' : 'bg-blue-400'
                  }`} 
                />
              )}
              ...
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-2">
                  <span className={`w-5 h-5 rounded-lg flex items-center justify-center text-[10px] font-mono font-bold shrink-0 ${
                    isActive
                      ? isEmployment ? 'bg-emerald-500 text-white' : 'bg-blue-500 text-white'
              ...
              <div className="flex items-center justify-between text-[11px] pl-7 text-slate-400">
                ...
                <span className={`text-[10px] font-mono truncate ml-1 px-1.5 py-0.2 rounded ${
                  sec.isCompleted 
                    ? 'text-emerald-300 bg-emerald-950/60 font-semibold' 
      ...
      <div className="p-3.5 border-t border-slate-800/80 bg-slate-950/60 space-y-2">
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className="flex items-center justify-center gap-1.5 px-2.5 py-2 bg-slate-800 hover:bg-slate-700 text-blue-300 hover:text-white rounded-xl text-[11px] font-bold uppercase tracking-wider border border-slate-700 transition-all cursor-pointer active:scale-95 shadow-2xs"
          ...
          <button
            ...
            className="flex items-center justify-center gap-1.5 px-2.5 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-xl text-[11px] font-bold uppercase tracking-wider border border-slate-700 transition-all cursor-pointer active:scale-95 disabled:opacity-50 shadow-2xs"
        ...
        {!isLocked && (
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider text-white shadow-lg transition-all cursor-pointer active:scale-95 ${
              isEmployment
